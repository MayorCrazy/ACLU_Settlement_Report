...
################### Histogram Intro ######################
histointro<-"Over the last two months, a growing portion of Philadelphians have experienced an increase in the number of days between commitment and admission to NSH. This is due to the fact that some Philadelphians are refusing to complete a COVID test while at the Philadephia Department of Prisons (PDP). For the safety of people at NSH, a COVID-19 test is a requirement for admission to NSH. When an individual at PDP does not agree to a COVID-19 test, he or she is placed on a medical hold. NSH requires people to have a COVID test in order to be scheduled for admission to NSH and upon arrival at NSH."
```
...
NSHForensic$Community<- case_when(NSHForensic$NSHLoc.y!="Philadelphia County Jail"~"Community",
                                  TRUE~"PDP")
NSHForensic$Community[NSHForensic$NSHLoc.y =='Wernersville State Hospital']<-'WeSH'
...
NSHCivil$Community<- case_when(NSHCivil$NSHLoc.y!="Philadelphia County Jail"~"Community",
                                  TRUE~"PDP")
NSHCivil$Community[NSHCivil$NSHLoc.y =='Wernersville State Hospital']<-'WeSH'
...
ComDC<-ComDC[which(ComDC$Community=="PDP"),]
...
ChartReviews<- ChartReviews %>% filter(as.Date(dateCompleted)<=enddt) %>% group_by(`ClientID#`) %>% summarize()%>% count()
...
### **As of February 2021, the definition of a step down was reviewed and updated. The updated definition generated new numbers for the individuals stepped down, which were first presented in the March 2021 report. The current definition of a step down is a lower level of care than the current placement. If an individual moves to jail or prison, it is not considered a lower level of care.  
