...
##################### Current WL ######################
ACLUAll[,11]<- as.Date(ACLUAll[,11])
ACLUAll[,12]<- as.Date.factor(ACLUAll[,12])
ACLUAll[,13]<- as.Date.factor(ACLUAll[,13])
ACLUAll[,14]<- as.Date.factor(ACLUAll[,14])
ACLUAll[,16]<- as.Date.factor(ACLUAll[,16])
...
AllWLDates <- ACLUAll %>% group_by(waitListEffDate) %>% summarise(n=n_distinct(`ClientID#`))
AllWLDates <- subset.data.frame(AllWLDates, select=-2)
AllWLDates$Qtr <-0
...
Facilities Admissions. ","As of ",format(as.Date(WLDate), '%m/%d/%y'), " Philadelphia's Forensic Waitlist for NSH stands at ",WLCTot,". The waitlist is composed of ",WLCFem," Females and ",WLCMal," Males.")
```
...
#Removed by Type All Time
WLARemoveTyp<-ACLUAll %>% group_by(removeHoldTypeName) %>% tally()#summarise(n= n_distinct(`ClientID#`))

############# Average WL Census for each Quarter with Plot ##############

WLADates<- ACLUAll[which((ACLUAll$genderName=="Male" &
                                              is.na(ACLUAll$removeHoldTypeName)==TRUE) |
                                             (ACLUAll$genderName=="Female" &
                                                is.na(ACLUAll$removeHoldTypeName))==TRUE)
                   ,]
...
ComunPlaceIntro<-paste0("The graph below shows a breakdown of all Philadelphians returned to the community from their hosting location as of ",format(WLDate,"%m/%d/%y"),". Since 1/27/19, ",sum(AlltoCommunity$n)," individuals have been discharged from NSH and PDP to the community.")
...
geom_text(mapping=aes(0,565),color="black",size=2.5,alpha = .4,family = "sans",hjust=0, label=paste0("- A total of ",docketindiv+162," individuals have received a docket \n  review since November of 2016.\n- As a result of this review, a total of 143 \n  individuals were then removed from the waitlist."))+
  ...
  ##################################### CHANGE THIS FIGURE MANUALLY ###############
  geom_text(mapping=aes(560,615),color="black",alpha = .4,size = 2.5,family = "sans",hjust=0, label=paste0("- A total of 338 individuals have received a chart \n  review since December of 2018."))+
...
TotalBeds<-c(24,6,14,14,12,12,3,3)
...
CRRTable = data.frame(matrix(nrow=7, ncol=6))
colnames(CRRTable)<-colnames(Stepdownsummary)
CRRTable$Placement<-c("HH - Farmhouse","Merakey - Fisher's Lane","RHD - Heart","RHD - Kailo","Carelink - STAR","Carelink - STAR Stepdown","SouthWest Recovery House")
CRRTable$`Total Beds`<-c(3,4,4,4,4,4,6)
CRRTable$LOC<-c("CRR","CRR","CRR","CRR","CRR","CRR","Recovery House")
...
PlacementTable<-bg(PlacementTable, bg='khaki1',15:15,)
